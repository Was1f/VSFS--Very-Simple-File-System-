...
int main(int argc, char *argv[]) {
    ...
    // Parsing the CLI parameters according to the question format
    ...
    // Validating the parameters as given limitations
    ...
    // Now Calculating the number of data blocks needed 
    ...
    // Accessing bitmaps and inode table
    ...
    // Find empty inode
    uint64_t max_inode_bits = sb->inode_count; 
    int free_inode_bit = find_first_free_bit(inode_bitmap, max_inode_bits, 1);  
    if (free_inode_bit == -1) {
    ...
    uint64_t new_inode_num = free_inode_bit + 1;  
    ...
    for (uint64_t i = 0; allocated_count < needed_blocks; i++) {  // Start from 0, but root may have taken some
        ...
        allocated_blocks[allocated_count++] = sb->data_region_start + free_data_bit;  
        i = free_data_bit;  
    }
    
    // creating the new inode with the last changes
    inode_t *new_inode = &inode_table[new_inode_num - 1];
    memset(new_inode, 0, INODE_SIZE);
    new_inode->mode = 0100000;  // file
    new_inode->links = 1;
    new_inode->uid = 0;
    new_inode->gid = 0;
    ...
    for (uint64_t i = 0; i < needed_blocks; i++) {
        ...
        if (copy_size == 0) copy_size = BS; 
        memcpy(block_ptr, file_content + i * BS, copy_size);
    }
    
    // updating root inode- Increased links by 1, update size and times
    root->links += 1;
    root->size_bytes += DIRENT_SIZE;
    root->mtime = (uint64_t)now;
    root->atime = (uint64_t)now;
    
    // finding available slot in root's data blocks and check for duplicate name
    int found_slot = 0;
    dirent64_t *new_de = NULL;
    for (int d = 0; d < DIRECT_MAX; d++) {

        if (root->direct[d] == 0) continue;  
        ...
        for (uint64_t e = 0; e < BS / DIRENT_SIZE; e++) {
            ...
            } else if (strncmp(de->name, filename, 58) == 0) {
    ...
    if (!found_slot) {
        ...
        memset(new_dir_block, 0, BS); 
        new_de = (dirent64_t *)new_dir_block;  
        root->size_bytes += BS;  
    }
